...
function preload() {
  patternShader = loadShader('shaders/pattern.vert?v=13', 'shaders/pattern.frag?v=13');
}
...
// ---- 入力処理（タッチ：将来のスマホ対応の下地。スクロールを抑止） ----

function touchStarted() {
  if (isOverUI(mouseY)) return; // UI 上のタッチは既定動作（ボタン）へ
  beginPointer();
  return false;
}
