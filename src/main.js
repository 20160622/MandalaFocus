...
// ---- 塗り続けるモード：着彩が退色せず、じっくり塗り絵として楽しめる ----
let paintHold = false;
...
function setup() {
  ...
  // 端末の回転に追従（resize が来ない/早すぎる端末への保険）。
  // 確定までに間があるので、少し遅らせて複数回フィットし直す。
  window.addEventListener('orientationchange', () => {
    setTimeout(fitToViewport, 100);
    setTimeout(fitToViewport, 400);
    setTimeout(fitToViewport, 800);
  });
}

function draw() {
  // 画面サイズのズレ（回転・URLバー開閉など）に毎フレーム追従させる保険
  reconcileViewport();
  ...
  patternShader.setUniform('u_paintHold', paintHold ? 1 : 0);
...
// キャンバスとストロークバッファを現在のビューポートサイズに合わせ直す。
// p5 の windowWidth は更新が遅れることがあるため、常に最新の innerWidth/Height を使う。
function fitToViewport() {
  const vw = window.innerWidth;
  const vh = window.innerHeight;
  if (width === vw && height === vh) return; // 変化なしなら何もしない（線を消さない）
  resizeCanvas(vw, vh);
  createStrokeLayer(width, height); // バッファも作り直す
}

// 毎フレーム、キャンバス寸法とビューポートのズレを監視して必ず追従させる。
// 回転直後は寸法が確定するまで揺れるので、安定して一定時間たってから合わせる。
let _vpPendingW = 0, _vpPendingH = 0, _vpStableAt = 0;
function reconcileViewport() {
  const vw = window.innerWidth;
  const vh = window.innerHeight;
  if (vw === width && vh === height) return; // 既に一致
  if (vw !== _vpPendingW || vh !== _vpPendingH) {
    // 寸法が動いている間は適用を先送り（安定を待つ）
    _vpPendingW = vw; _vpPendingH = vh;
    _vpStableAt = millis() + 150;
  } else if (millis() >= _vpStableAt) {
    fitToViewport();
  }
}

function windowResized() {
  // イベント駆動でも一応合わせる（reconcileViewport が最終的な保険）
  fitToViewport();
}
...
// 塗り続けるモードの切替（オンの間は着彩が退色しない）
function togglePaintHold() {
  paintHold = !paintHold;
  setPaintHold(paintHold);
  const btn = document.getElementById('btn-paint');
  if (btn) {
    btn.textContent = paintHold ? '🖌 塗り続け中' : '🖌 塗り続け';
    btn.classList.toggle('active', paintHold);
  }
  if (paintHold) {
    showToast('塗り続けるモード：色が消えません');
  } else {
    fadeOutHeldTaps(); // 解除したら、保持していた色をいまから退色させる
    showToast('塗り続けるモード：オフ（色が退きます）');
  }
}
...
function keyPressed() {
  ...
  else if (key === 'b' || key === 'B') togglePaintHold();
...
function setupUI() {
  ...
  document.getElementById('btn-paint').addEventListener('click', togglePaintHold);
